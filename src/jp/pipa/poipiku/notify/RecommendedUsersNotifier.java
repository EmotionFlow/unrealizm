	...
	public boolean notifyToLongSilenceUser(DataSource dataSource) {
		...
		try {
			...
			sql = "SELECT user_id, email, nickname, lang_id, last_login_date" +
					...
					" ORDER BY last_dm_delivery_date DESC NULLS FIRST, last_login_date DESC LIMIT 400";
			...
			for (User targetUser : deliveryTargets) {
				// Twitterでフォローしているが、フォローしていないユーザー
				// Twitterでフォローしているが、フォローしていないユーザーのうち、エアスケブ募集しているユーザー
				sql = "WITH" +
