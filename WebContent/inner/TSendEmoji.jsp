				...
				$.ajax({
						...
						"dataType": "json",
								...
								if (data.result_num > 0) {
										...
										case <%=SendEmojiC.ERR_RETRY%>:
												DispMsg("<%=_TEX.T("CheerDlg.Err.CardAuth")%>");
												break;
										case <%=SendEmojiC.ERR_INQUIRY%>:
												alert("<%=_TEX.T("CheerDlg.Err.AuthCritical")%>");
												break;
										case <%=SendEmojiC.ERR_CARD_AUTH%>:
												DispMsg("<%=_TEX.T("CheerDlg.Err.CardAuth")%>");
												break;
										case <%=SendEmojiC.ERR_MAX_EMOJI%>:
											DispMsg("<%=_TEX.T("CheerDlg.Err.EmojiMax")%>");
											break;
										case <%=SendEmojiC.ERR_UNKNOWN%>:
