...
int m_nCategoryId = Util.toIntN(request.getParameter("CAT"), 0, Common.CATEGORY_ID_MAX);
String m_strDescription = Common.TrimAll(Util.toString(request.getParameter("DES")));
String m_strTagList = Common.SubStrNum(Common.TrimAll(Util.toString(request.getParameter("TAG"))), 100);
int m_nMode = Util.toInt(request.getParameter("MOD"));
...
if(checkLogin.m_bLogin && (checkLogin.m_nUserId == m_nUserId)) {
	...
	editor_id = cResSet.getInt("editor_id");
		...
		cState.setString(2, Common.SubStrNum(m_strDescription, Common.EDITOR_DESC_MAX[editor_id][checkLogin.m_nPassportId]));
...
{
"result": <%=nRtn%>,
"html" : "<%=CEnc.E(Common.AutoLink(Util.toStringHtml(m_strDescription), m_nUserId, m_nMode))%>",
"text" : "<%=CEnc.E(m_strDescription)%>",
"htmlTag" : "<%=CEnc.E(Common.AutoLink(Util.toStringHtml(m_strTagList), m_nUserId, m_nMode))%>",
