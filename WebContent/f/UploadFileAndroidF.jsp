...
<%
	...
	if(strName.equals("UID")) {
		...
		m_nCategoryId = Util.toIntN(item.getString(), 0, 16);
	} else if(strName.equals("DES")) {
		m_strDescription = Common.TrimAll(Util.toString(item.getString("UTF-8")));
		...
		m_bTweet = (Util.toInt(item.getString())==1);
	} else if(strName.equals("REC")) {
		m_nOpenId = Util.toIntN(item.getString(), 0, 2);
	} else if(strName.equals("IMG")){
		m_nOptImage = Util.toIntN(item.getString(), 0, 1);
	}
	item.delete();
		} else {
	item_file = item;
