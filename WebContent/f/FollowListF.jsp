	...
	public boolean GetResults(FollowListCParam cParam) {
		...
		try {
			...
			if(cParam.m_nStartId>0) {
				...
				strSql = "SELECT follows_0000.*, nickname, file_name FROM follows_0000 INNER JOIN users_0000 ON follows_0000.follow_user_id=users_0000.user_id WHERE follows_0000.user_id=? AND follow_id<? ORDER BY follow_id DESC LIMIT ?";
				cState = cConn.prepareStatement(strSql);
				cState.setInt(1, cParam.m_nAccessUserId);
				cState.setInt(3, cParam.m_nStartId);
				cState.setInt(3, cParam.m_nSelectMax);
			} else {
				strSql = "SELECT follows_0000.*, nickname, file_name FROM follows_0000 INNER JOIN users_0000 ON follows_0000.follow_user_id=users_0000.user_id WHERE follows_0000.user_id=? ORDER BY follow_id DESC LIMIT ?";
				cState = cConn.prepareStatement(strSql);
				cState.setInt(1, cParam.m_nAccessUserId);
