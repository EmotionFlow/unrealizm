...
%><%!
class SearchIllustByKeywordCParam {
...
class SearchIllustByKeywordC {
	public Vector<CContent> m_vContentList = new Vector<CContent>();
	int m_nEndId = -1;

	public boolean GetResults(SearchIllustByKeywordCParam cParam) {
		...
		try {
			...
			if(cParam.m_nStartId>0) {
				...
				strSql = "SELECT * FROM contents_0000 WHERE content_id<? AND description ILIKE ? AND user_id NOT IN(SELECT block_user_id FROM blocks_0000 WHERE user_id=?) AND user_id NOT IN(SELECT user_id FROM blocks_0000 WHERE block_user_id=?) ORDER BY content_id DESC LIMIT ?";
				...
				strSql = "SELECT * FROM contents_0000 WHERE description ILIKE ? AND user_id NOT IN(SELECT block_user_id FROM blocks_0000 WHERE user_id=?) AND user_id NOT IN(SELECT user_id FROM blocks_0000 WHERE block_user_id=?) ORDER BY content_id DESC LIMIT ?";
				cState = cConn.prepareStatement(strSql);
