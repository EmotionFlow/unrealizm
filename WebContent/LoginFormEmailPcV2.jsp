	...
	<head>
		...
		<script>
			function RegistUser() {
				...
				grecaptcha.ready( () => {
					grecaptcha.execute('<%=ReCAPTCHA.SITE_KEY%>', {action: 'contact_form'}).then((reCAPTCHAtoken) => {
						$('#recaptcha-response').val(reCAPTCHAtoken);
						$.ajaxSingle({
							"type": "post",
							"data": {
								"NN":strNickname,
